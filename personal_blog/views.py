...
from personal_blog.forms import ContactForm, NewsLetterForm, PostForm, CommentForm
...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        obj.save()
        context = super().get_context_data(**kwargs)
        context['previous_post'] = Post.objects.filter(status='published', id__lt=obj.id).order_by('-id').first()
        context['next_post'] = Post.objects.filter(status='published', id__gt=obj.id).order_by('id').first()
        return context
        ...
        if is_ajax == "XMLHttpRequest":  # ajax or not
            ...
            if form.is_valid():
                ...
                return JsonResponse({"success": True,
                    "message": "Congratulation, you have successfully subscribed to out newsletter"}, 
                    status=200)
...
class CommentCreateView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            return redirect('post-detail', request.POST["post"])
        else:
